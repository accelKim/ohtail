...
const Signup = () => {
    ...
    const handleEmailCheck = async () => {
        try {
            const response = await axios.post(`${apiUrl}/api/check-email`, { email });
    ...
    const handleNicknameCheck = async () => {
        try {
            const response = await axios.post(`${apiUrl}/api/check-nickname`, { nickname });
    ...
    return (
        <div className={`mw ${style.signup}`}>
            ...
            <form onSubmit={onSubmit}>
                ...
                <label>비밀번호 확인</label>
                <input type="password" value={passwordcon} onChange={(e) => setPasswordcon(e.target.value)} required />
                ...
                <label>음주 빈도</label>
                <select value={drinkingFrequency} onChange={(e) => setDrinkingFrequency(e.target.value)} required>
                ...
                {errMessage.drinkingFrequency && (
                    <div className={style.errorMessage}>{errMessage.drinkingFrequency}</div>
                )}

                <label>선호 재료</label>
                <select value={preferredIngredients} onChange={(e) => setPreferredIngredients(e.target.value)} required>
                    <option value="">선택해주세요</option>
