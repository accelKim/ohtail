...
const RecipeListPage = () => {
    ...
    useEffect(() => {
        const fetchOptions = async () => {
            try {
                const degreeResponse = await axios.get('https://www.thecocktaildb.com/api/json/v1/1/list.php?c=list');
                ...
                setDegreeOptions(
                    degreeResponse.data.drinks.map((drink) => ({ value: drink.strCategory, label: drink.strCategory }))
                );
                setGlassOptions(
                    glassResponse.data.drinks.map((drink) => ({ value: drink.strGlass, label: drink.strGlass }))
                );
    ...
    return (
        <div className="mw">
            ...
            />
            <RecipeList searchResults={currentResults} hasSearched={hasSearched || initialRecipes.length > 0} />
            <Pagination
