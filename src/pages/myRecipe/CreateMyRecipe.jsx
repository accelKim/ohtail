...
const CreateMyRecipe = () => {
  ...
  useEffect(() => {
    ...
    const handleCreateRecipe = async (e) => {
        ...
        if (
            ingredients.some(
                (ingredient) => ingredient.originalName === '' || ingredient.quantity === '' || ingredient.unit === ''
            )
        ) {
            alert('재료를 입력해주세요');
            return;
        ...
        try {
            const response = await fetch(`${apiUrl}/createMyRecipe`, {
                method: 'POST',
                headers: {
    ...
    const handleSearchChange = (e, index) => {
        ...
        const filtered = translatedIngredientOptions.filter((option) => option.toLowerCase().includes(searchValue));
        ...
        <main className={`mw ${style.main}`}>
            ...
            <form onSubmit={handleCreateRecipe} className={style.form}>
                ...
                <div className={style.ingredientsCon}>
                                        ...
                                        {ingredient.filteredOptions.length > 0 &&
                                            ingredient.filteredOptions.map((option, i) => (
                                                <div
                                                    key={i}
                                                    onClick={() =>
                                                        handleOptionClick(index, option, ingredient.originalOptions[i])
                                                    }
                    ...
                    <button type="button" onClick={handleAddIngredient} className={style.addIngredientBtn}>
                        재료 추가
                    </button>
                </div>
                <div className={style.instructionCon}>
                    ...
                    <div className={style.inputWrapper}>
                        <textarea
                            ...
                            ref={textareaRef}
                        ></textarea>
                        <div className={style.charCount_instruct}>{instructions.length}/300</div>
                    </div>
                </div>
