...
const Calculator = () => {
  ...
  const calculateABV = () => {
    const totalVolume = ingredients.reduce(
      (acc, ingredient) => acc + parseFloat(ingredient.volume || 0),
      0
    );
    const totalAlcohol = ingredients.reduce(
      (acc, ingredient) =>
        acc +
        (parseFloat(ingredient.volume || 0) * parseFloat(ingredient.abv || 0)) /
          100,
      0
    );
  ...
  return (
    <div>
      ...
      <button onClick={handleAddIngredient} className={style.button}>
        재료 추가
      </button>
      <button onClick={calculateABV} className={style.button}>
        도수 계산
      </button>
      {totalABV !== null && (
        <div className={style.calculator_result_wrap}>
          <h3>
            칵테일 도수: <span>{totalABV}</span>%
          </h3>
        </div>
      )}
    </div>
